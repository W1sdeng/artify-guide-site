...
function validateUrl(url) {
    ...
    if (!url.hostname)
        throw new InvalidLinkError("That link needs a hostname.");
...
function getFamilyIdentity(hostname) {
    return getDomain(hostname, { allowPrivateDomains: true }) ?? hostname;
}
export function parseLink(input, options = {}) {
    const version = options.identityVersion ?? IDENTITY_VERSION;
    const scope = options.identityScope ?? "site";
    if (version !== IDENTITY_VERSION)
        throw new UnsupportedVersionError("identity", version);
    ...
    if (!normalizedInput)
        throw new InvalidLinkError("Enter a link to discover its form.");
    let url;
    try {
        url = new URL(withDefaultScheme(normalizedInput));
    }
    catch (error) {
        if (error instanceof EveryQRCodeError)
            throw error;
        throw new InvalidLinkError("That link does not look valid yet.");
    }
    validateUrl(url);
