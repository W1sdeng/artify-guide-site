...
export const QR_PROFILE_V1 = Object.freeze({
...
function copyCells(code) {
    ...
    for (let row = 0; row < code.modules.size; row += 1) {
        for (let column = 0; column < code.modules.size; column += 1) {
            ...
            if (value !== 0 && value !== 1) {
                throw new EveryQRCodeError("generation-failed", "The QR encoder returned an invalid module.");
            }
...
function validateEncoderResult(code) {
    ...
    if (code.version > QR_PROFILE_V1.maxSymbolVersion)
        throw new LinkTooComplexError();
    if (code.modules.size !== 17 + 4 * code.version) {
        throw new EveryQRCodeError("generation-failed", "The QR encoder returned an invalid matrix size.");
    }
    if (code.modules.data.length !== code.modules.size ** 2) {
        throw new EveryQRCodeError("generation-failed", "The QR encoder returned incomplete module data.");
    }
...
export function createQRMatrix(payloadUrl, profileVersion = 1) {
...
export function qrCell(matrix, column, row) {
