import { mixFamily, resolveGeneratorVersion, } from "@every-qrcode/core";
...
export const TREE_ARCHETYPE_WEIGHTS = {
    banana: 0.08,
    cloud: 0.15,
    conifer: 0.12,
    "multi-trunk": 0.12,
...
const TREE_ARCHETYPE_SELECTION_ORDER = [
    "round",
    "cloud",
    "conifer",
    "umbrella",
...
const NAME_ENDINGS = ["Haven", "Reach", "Drift", "Basin", "Arc", "Garden", "Crown"];
function selectTreeArchetype(gene) {
    let threshold = 0;
...
function pushSeedBlock(blocks, column, row, baseY, type) {
    blocks.push({
        baseY,
        column,
        index: blocks.length,
...
function baseBlockType(active, radiusSquared, canopySquared) {
    if (!active)
        return SEED_BLOCK_TYPES.dirt;
    if (radiusSquared < 6.25)
        return SEED_BLOCK_TYPES.cherryBlossom;
    if (radiusSquared >= canopySquared)
        return SEED_BLOCK_TYPES.grass;
    return SEED_BLOCK_TYPES.fallenPetals;
}
function canopyLayerNoise(column, row, salt) {
    const value = Math.sin(column * 127.1 + row * 311.7 + salt * 43.7) * 43_758.5;
    return value - Math.floor(value);
}
function addSeedColumnLayers(blocks, column, row, radiusSquared, qrSize) {
    ...
    if (radiusSquared >= canopyRadius ** 2)
        return;
    ...
    for (let layer = 0; layer < blossomLevels + extraLevels; layer++) {
        pushSeedBlock(blocks, column, row, canopyBaseY + layer * SEED_BLOCK_SIZE + heightJitter, SEED_BLOCK_TYPES.cherryBlossom);
    }
    if (radiusSquared >= 2.25)
        return;
    for (let layer = 0; layer < 4; layer++) {
        pushSeedBlock(blocks, column, row, canopyBaseY + layer * SEED_BLOCK_SIZE, SEED_BLOCK_TYPES.cherryBlossom);
    }
}
function smoothTerrain(source, size, passes) {
    let current = source;
...
function terrainEdgeMask(column, row, size) {
...
function createTerrainHeights(model) {
...
function packSeedBlockField(blocks, qrSize, terrainHeights) {
    const activeBaseCells = new Set(blocks
...
export function createSeedBlockField(model, form = "tree") {
...
function pointForTopology(topology, u, v, relief, eccentricity) {
...
function createModuleAnchors(identity, topology, eccentricity) {
    const modules = [];
...
function selectFeatureAnchors(identity, topology, eccentricity, stream) {
    const size = identity.qr.size;
    const candidates = Array.from(identity.qr.cells, (_, index) => {
        const score = (identity.fields.blur[index] ?? 0) * 0.62 +
    ...
    for (const candidate of candidates) {
        ...
        const tooClose = selected.some((feature) => {
            const dot = feature.normal[0] * point.normal[0] +
...
function createSatellites(stream) {
...
async function createSeedModelV1(identity) {
    const detailSource = identity.link.scope === "url" ? "page" : "site";
    const [family, familyShape, detailShape, detail, familyMorph, detailMorph, archetype] = await Promise.all([
    ...
    if (!recipe)
        throw new RangeError("Tree recipe");
...
export async function createSeedModel(identity, options = {}) {
