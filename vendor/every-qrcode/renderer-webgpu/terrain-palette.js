...
function targetLuminance(color, target) {
    const current = luminance(color);
    if (Math.abs(current - target) < 0.000_1)
        return color;
    if (current < target) {
...
function terrainTone(color, target, saturation) {
    return targetLuminance(saturate(color, saturation), target);
}
export function createTerrainPalette(theme) {
