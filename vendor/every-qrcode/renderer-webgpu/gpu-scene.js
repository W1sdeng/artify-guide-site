...
function qrEdgeRatio(model) {
    ...
    for (let row = 0; row < model.qrSize; row++) {
        for (let column = 0; column < model.qrSize; column++) {
            ...
            if (column > 0 && active.has(index) !== active.has(index - 1))
                changes++;
            if (row > 0 && active.has(index) !== active.has(index - model.qrSize))
                changes++;
        }
    }
    return changes / (2 * model.qrSize * (model.qrSize - 1));
}
function canopyProfile(model) {
...
export function baseTrunkHeight(density, sizeScale) {
    return (0.29 + density * 0.07) * sizeScale;
}
function archetypeBranchCount(archetype, gene) {
    if (archetype === "banana")
        return 9 + Math.floor(gene * 3);
    if (archetype === "conifer")
        return 6;
    if (archetype === "multi-trunk")
        return 7 + Math.floor(gene * 2);
    if (archetype === "umbrella")
        return 6 + Math.floor(gene * 2);
    if (archetype === "willow")
        return 8 + Math.floor(gene * 2);
    if (archetype === "windswept")
        return 5 + Math.floor(gene * 2);
    if (archetype === "cloud")
        return 9 + Math.floor(gene * 2);
    return 7 + Math.floor(gene * 2);
}
function measureTree(model) {
    ...
    const trunkHeight = baseTrunkHeight(density, sizeScale) * profile.trunk * (0.98 + heightGene * 0.18);
    ...
    const offsetDistance = model.archetype === "windswept"
...
function trunkPoint(metrics, progress) {
    return [
...
function addTrunk(segments, metrics) {
...
function addConiferTree(segments, tips, metrics, seed) {
...
function addBananaTree(segments, tips, metrics, seed) {
    addTrunk(segments, metrics);
...
function forkPoint(root, crown, progress) {
    return [
...
function addSharedForkBase(segments, metrics) {
    const root = [0, 0, 0];
...
function addForkedTree(segments, tips, metrics, seed) {
...
function addForkedCrown(segments, tips, metrics, seed, trunk, crown, radius) {
...
function constrainToCanopy(point, metrics) {
...
function addTwigBurst(segments, tips, metrics, spec) {
    pushSegment(segments, {
        depth: spec.depth,
        end: spec.end,
    ...
    for (let twig = 0; twig < twigCount; twig++) {
        ...
        const length = metrics.canopyRadius *
        ...
        const elevation = metrics.archetype === "willow" ? -0.42 + elevationGene * 0.18 : 0.1 + 0.45 * elevationGene;
        const radius = spec.endRadius * (0.5 + 0.2 * random(spec.seed, segments.length, twig, 800));
        const end = constrainToCanopy(add(spec.end, scale(direction(azimuth, elevation), length)), metrics);
        ...
        const secondElevation = metrics.archetype === "willow"
        ...
        const secondEnd = constrainToCanopy(add(end, scale(direction(secondAzimuth, secondElevation), secondLength)), metrics);
        pushSegment(segments, {
            depth: spec.depth + 2,
            end: secondEnd,
...
function addMainBranch(segments, tips, metrics, seed, branch) {
    const start = trunkPoint(metrics, 0.7 + 0.25 * random(seed, branch, 0, 1_200));
    const azimuth = (branch / metrics.branchCount) * Math.PI * 2 + 0.4 * (random(seed, branch, 0, 900) - 0.5);
...
function addCrownBranch(segments, tips, metrics, spec) {
    const start = trunkPoint(metrics, 0.95);
    const azimuth = (spec.index / spec.count) * Math.PI * 2 + 0.5 * random(spec.seed, spec.index, 0, 2_100);
...
function toGridPoint(model, x, z) {
    const halfGrid = model.qrSize * SEED_BLOCK_SIZE * 0.5;
    return [(x + halfGrid) / SEED_BLOCK_SIZE, (z + halfGrid) / SEED_BLOCK_SIZE];
}
function addTipCluster(flowers, model, appearance, tip, tipIndex) {
    const seed = sceneSeed(model);
    if (random(seed, tipIndex, 0, 780) < 0.01)
        return;
    const halfGrid = model.qrSize * SEED_BLOCK_SIZE * 0.5;
    ...
    if (random(seed, tipIndex, 2, 900) < appearance.leafDensity) {
        ...
        const [column, row] = toGridPoint(model, tip.position[0] + Math.cos(angle) * clusterRadius * 0.4, tip.position[2] + Math.sin(angle) * clusterRadius * 0.4);
        flowers.push(column, row, tip.position[1] - SEED_BLOCK_SIZE * 0.5, 1 + random(seed, tipIndex));
    }
}
function createCanopyLobe(field, lobe, innerRadius = 0.04) {
    return {
...
function roundCanopyFields(field) {
    return [
...
function cloudCanopyFields(field) {
    return [
        createCanopyLobe(field, {
            depth: 0.72,
            height: 0.34,
...
function windsweptCanopyFields(field) {
    return [
        createCanopyLobe(field, {
            depth: 0.68,
            height: 0.58,
...
function willowCanopyFields(field) {
    return [
        createCanopyLobe(field, {
            depth: 0.78,
            height: 0.78,
...
function umbrellaCanopyFields(field) {
    return [
        createCanopyLobe(field, {
            depth: 0.94,
            height: 0.86,
...
function multiTrunkCanopyFields(field, seed) {
...
function createCanopyFields(model, metrics) {
...
function canopyHeightRange(metrics, field, normalizedRadius) {
    ...
    const sphere = Math.max(0, 1 - normalizedRadius ** 2) ** (1 / Math.max(1, metrics.canopyRoundness));
    ...
    const terrace = Math.floor(normalizedRadius * 4) * metrics.canopyTiering * field.domeHeight * 0.08;
...
function omitsInteriorCanopyColumn(model, metrics, column, row) {
    const center = model.qrSize * 0.5;
    const normalizedX = (column - center - metrics.canopyOffsetX / SEED_BLOCK_SIZE) /
        (metrics.canopyRadius / SEED_BLOCK_SIZE);
    const normalizedZ = (row - center - metrics.canopyOffsetZ / SEED_BLOCK_SIZE) /
        (metrics.canopyDepth / SEED_BLOCK_SIZE);
    if (Math.hypot(normalizedX, normalizedZ) >= 0.72)
        return false;
...
function addCanopyField(flowers, model, metrics, field) {
    const seed = sceneSeed(model);
    for (let row = 0; row < model.qrSize; row++) {
        for (let column = 0; column < model.qrSize; column++) {
            if (omitsInteriorCanopyColumn(model, metrics, column, row))
                continue;
            ...
            if (normalizedRadius >= 1 || normalizedRadius < field.innerRadius)
                continue;
            ...
            for (let layer = 0; layer < layerCount; layer++) {
                ...
                for (let organ = 0; organ < organCount; organ++) {
                    flowers.push(column + 1.2 * (random(seed, column, organ, 700 + row + field.salt) - 0.5), row + 1.2 * (random(seed, row, organ, 800 + column + field.salt) - 0.5), height + (random(seed, column + row, organ, 900 + field.salt) - 0.5) * SEED_BLOCK_SIZE, random(seed, column, row, 1_000 + organ + layer * 10 + field.salt));
                }
            }
            if (random(seed, column, row, 1_100) >= 0.35)
                continue;
            flowers.push(column + 0.5 * (random(seed, column, row, 1_200) - 0.5), row + 0.5 * (random(seed, row, column, 1_300) - 0.5), mix(bottomY, topY, 0.45), 1 + random(seed, column, row, 1_400));
        }
    }
}
function addBananaFronds(flowers, model, metrics) {
    const seed = sceneSeed(model);
    const center = model.qrSize * 0.5;
    ...
    for (let frond = 0; frond < frondCount; frond++) {
        ...
        for (let step = 0; step < steps; step++) {
            ...
            const height = metrics.trunkHeight * 1.08 +
            ...
            for (let cross = 0; cross < crossSections; cross++) {
                ...
                flowers.push(centerColumn - Math.sin(angle) * lateral, centerRow + Math.cos(angle) * lateral, height + (random(seed, frond, step, 6_700 + cross) - 0.5) * SEED_BLOCK_SIZE, 5 + random(seed, frond, step, 6_800 + cross) * 0.99);
            }
        }
    }
}
function addWillowCurtains(flowers, model, metrics) {
    const seed = sceneSeed(model);
    const center = model.qrSize * 0.5;
    ...
    for (let curtain = 0; curtain < curtainCount; curtain++) {
        ...
        const radius = (metrics.canopyRadius / SEED_BLOCK_SIZE) * (0.58 + random(seed, curtain, 0, 7_100) * 0.28);
        const top = metrics.canopyBaseY +
        ...
        for (let step = 0; step < steps; step++) {
            ...
            flowers.push(center + Math.cos(angle) * radius + Math.sin(progress * Math.PI) * 0.18, center + Math.sin(angle) * radius, top - drop * progress, 1 + random(seed, curtain, step, 7_400));
        }
    }
}
export function supportsCanopyFringe(archetype) {
    return archetype !== "banana" && archetype !== "conifer" && archetype !== "willow";
}
function addCanopyFringe(flowers, model, metrics) {
    const seed = sceneSeed(model);
    ...
    for (let fringe = 0; fringe < fringeCount; fringe++) {
        ...
        const radius = (metrics.canopyRadius / SEED_BLOCK_SIZE) * (0.68 + random(seed, fringe, 0, 7_600) * 0.24);
        const top = metrics.canopyBaseY +
        ...
        for (let step = 0; step < steps; step++) {
            ...
            flowers.push(centerColumn + Math.cos(angle) * (radius - inwardCurve), centerRow + Math.sin(angle) * (radius - inwardCurve), top - drop * progress, random(seed, fringe, step, 8_000));
        }
    }
}
function addCanopySurface(flowers, model, metrics) {
    ...
    if (metrics.archetype === "willow")
        addWillowCurtains(flowers, model, metrics);
    if (supportsCanopyFringe(metrics.archetype))
        addCanopyFringe(flowers, model, metrics);
}
function removeInteriorCanopyColumns(flowers, model, metrics) {
    if (metrics.archetype === "banana" || metrics.archetype === "conifer")
        return [...flowers];
    const filtered = [];
    for (let offset = 0; offset < flowers.length; offset += 4) {
        ...
        if (omitsInteriorCanopyColumn(model, metrics, column, row))
            continue;
        filtered.push(flowers[offset] ?? 0, flowers[offset + 1] ?? 0, flowers[offset + 2] ?? 0, flowers[offset + 3] ?? 0);
    }
    return filtered;
}
function createGroundPetals(model, appearance) {
    const petals = [];
    const seed = sceneSeed(model);
...
function createFallingPetals(tips, model) {
    if (tips.length === 0)
    ...
    for (let index = 0; index < count; index++) {
        ...
        petals.push((x + halfGrid) / SEED_BLOCK_SIZE, (z + halfGrid) / SEED_BLOCK_SIZE, topY, random(seed, index, 0, 11_400));
    }
    return new Float32Array(petals);
}
function createRain(model) {
...
function createButterflies(model) {
...
function createFlowers(model, tips, appearance, metrics) {
    const flowers = [];
    if (model.archetype === "banana") {
        addCanopySurface(flowers, model, metrics);
        return packOrgans(flowers);
    }
    const clusterTips = model.archetype === "round"
        ? tips.filter((tip) => tip.position[1] < metrics.canopyBaseY + metrics.canopyDomeHeight * 0.72)
        : tips;
...
function createGrass(model) {
    ...
    for (const module of model.modules) {
        ...
        if (radiusSquared < canopyRadiusSquared)
            continue;
...
function createTerrainScene(model, appearance) {
...
function assembleScene(model, appearance, organs, segments, fallingPetals) {
...
export function createSeedGpuScene(model, form = "tree") {
    ...
    return assembleScene(model, appearance, organs, tree.segments, createFallingPetals(tree.tips, model));
}
